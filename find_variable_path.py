...
def fuzzy_search_scenario_steps(matches_dict, scenario_steps):
    ...
    for current_scenario in scenario_steps:
        for step in current_scenario:
            ...
            if result[1]>80: # Set fuzzy threshold
                fuzzy_match.append([current_scenario,step,result[0]])
...
def select_json_file():
    """Opens a file dialog to select json file."""
    root = tk.Tk()
...
# --- Your existing functions (fuzzy_search_scenario_steps, constants, select_json_file, load_training_data, train_and_save_model, load_model_components) go here ---
# They are not modified in this request, but are needed for context.
...
def fuzzy_search_scenario_steps(matches_dict, scenario_steps):
    ...
    for current_scenario in scenario_steps:
        for step in current_scenario:
            ...
            result = process.extractOne(lowercase_step, list(matches_dict.keys()), scorer=fuzz.token_set_ratio)
            if result[1]>80: # Set fuzzy threshold
                fuzzy_match.append([current_scenario,step,matches_dict[result]])
    return fuzzy_match
